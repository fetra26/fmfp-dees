<?php

namespace App\Filament\Widgets;

use App\Models\Projet;
use Filament\Widgets\ChartWidget;
use Illuminate\Support\Facades\Cache;

class RepartitionSecteurChart extends ChartWidget
{
    use Concerns\CheckDashboardVisibility;

    public const SLUG = 'repartition_secteur';

    protected ?string $heading = '🏭 Répartition par secteur';
    protected static ?int $sort = 7;
    protected static bool $isLazy = true;

    protected function getData(): array
    {
        return Cache::remember('widget.chart.secteurs', 300, function () {
            $data = Projet::join('secteur', 'projet.secteur_id', '=', 'secteur.id')
                ->selectRaw('secteur.libelle as secteur, COUNT(*) as total')
                ->groupBy('secteur.libelle')
                ->orderByDesc('total')
                ->limit(10)
                ->get();

            return [
                'datasets' => [[
                    'label'           => 'Projets',
                    'data'            => $data->pluck('total')->toArray(),
                    'backgroundColor' => ['#3b82f6','#10b981','#f59e0b','#ef4444','#8b5cf6','#06b6d4','#84cc16','#f97316','#ec4899','#6b7280'],
                ]],
                'labels' => $data->pluck('secteur')->toArray(),
            ];
        });
    }

    protected function getType(): string { return 'pie'; }
}
